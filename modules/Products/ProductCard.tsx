...
const ProductCard: FC<{ item: ProductType }> = ({ item }) => {
  ...
  return (
    <div id='products_card' onClick={(e) => (e.target as HTMLDivElement).id == "products_card" && router.push(`/products/${item.id}`)} className='max-w-[273px] slideList'>
      <div className='bg-[#EBEFF3] !object-cover cursor-pointer rounded-[8px] mb-4 relative h-[180px] sm:h-[280px] flex items-center justify-center'>
        <Image id='products_card' className='hover:scale-[1.1] duration-200 w-[130px] h-[130px] !object-contain sm:w-[180px] sm:!h-[180px]' src={`${IMG_API}/${item.image}`} alt='Product img' width={202} height={202} priority />
        <button className='absolute top-[12px] sm:top-[20px] right-[12px] sm:right-[24px] cursor-pointer'> <LikeIcon/> </button>
      </div>
      <p id='products_card' className='cursor-pointer hover:text-[#134E9B] duration-200 text-[#545D6A] !line-clamp-1 sm:line-clamp-none text-[12px] sm:text-[14px] mb-[20px] sm:mb-[28px]'>{item.description}</p>
      <div className='flex flex-col sm:flex-row w-full justify-between items-start sm:items-center'>
        <div className='flex flex-col'>
          <strong id='products_card' className='cursor-pointer hover:text-[#134E9B] duration-200 text-[#0A1729] text-[15px] sm:text-[20px]'>{formatNumber(item.price)} usz</strong>
        ...
        <div className='flex gap-[10px] w-full sm:w-auto mt-[16px] sm:mt-[0px]'>
          ...
          <Button icon={<SavedIcon />} iconPosition='right' extraClass='sm:hidden !w-[60%] text-[12px] !font-normal !py-[2px] !gap-[6px]'> {t("basket")} </Button>
        </div>
      </div>
    </div>
  )
